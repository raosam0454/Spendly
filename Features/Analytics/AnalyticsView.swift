            ...
            if viewModel.categoryChartData.isEmpty {
                ...
                .chartXAxis {
                    AxisMarks(position: .bottom) {
                        let value = $0.as(Double.self) ?? 0
                        AxisValueLabel {
                            Text((value * settings.conversionRate).formatted(.currency(code: settings.selectedCurrencyCode)))
                        }
                    }
                }
            }
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            VStack(alignment: .leading) {
                                ...
                                Text(viewModel.totalExpenses * settings.conversionRate, format: .currency(code: settings.selectedCurrencyCode))
                                    .font(.subheadline.bold())
                            }
                        }
                    }
    ...
    private var monthComparisonCard: some View {
        HStack {
            ...
            VStack(alignment: .trailing) {
                ...
                Text(viewModel.totalIncome * settings.conversionRate, format: .currency(code: settings.selectedCurrencyCode))
    ...
    private var highestSpendingCard: some View {
        if let (day, amount) = viewModel.highestSpendingDay, amount > 0 {
            HStack {
                ...
                Spacer()
                Text(amount*settings.conversionRate, format: .currency(code: settings.selectedCurrencyCode))
